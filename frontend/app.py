...
def convert_report_to_html(markdown_text: str, topic: str) -> str:
    """Convert markdown report to styled HTML for download."""
    html = markdown_text
...
<head>
    ...
    <style>
        ...
        h1 {{ color: #1a1a2e; border-bottom: 2px solid #4a9eff;
             padding-bottom: 8px; }}
...
<body>
    <p>{html}</p>
    <footer style="margin-top:3em;padding-top:1em;border-top:1px solid #ddd;
                    color:#999;font-size:0.8em;">
        Generated by NewsForge Multi-Agent Research Pipeline
    </footer>
</body>
</html>"""
...
if results and results.get("draft_report") and not st.session_state.awaiting_approval:
    ...
    dl_col1, dl_col2, dl_col3, dl_col4, dl_col5 = st.columns([1, 1, 1, 1, 1])
    with dl_col1:
        st.download_button(
    ...
    with dl_col2:
        html_report = convert_report_to_html(draft, results.get("topic", "Report"))
        st.download_button(
            label="Download HTML",
            data=html_report,
            file_name=f"newsforge-{t_slug}.html",
            mime="text/html",
            use_container_width=True,
        )
    with dl_col3:
